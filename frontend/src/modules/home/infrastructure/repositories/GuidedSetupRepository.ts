...
export class GuidedSetupRepository extends BaseRepository implements IGuidedSetupRepository {
  ...
  async getSetupState(): Promise<GuidedSetupState> {
    return this.get<GuidedSetupState>('/setup', 'Failed to load setup state');
  }

  async updateStepStatus(stepId: SetupStepId, status: 'completed' | 'skipped'): Promise<GuidedSetupState> {
    return this.patch<GuidedSetupState>(
      `/setup/steps/${stepId}`,
      { status },
  ...
  async resetSetup(): Promise<GuidedSetupState> {
    return this.post<GuidedSetupState>('/setup/reset', undefined, 'Failed to reset setup');
  }
}
