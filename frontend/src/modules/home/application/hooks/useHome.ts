...
export function useUpdateChecklistItem() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedItem) => {
      // Update only the checklist data in the dashboard query cache
      // This avoids refetching the entire dashboard
      queryClient.setQueryData(QUERY_KEYS.dashboard, (oldData: HomeDashboard) => {
        if (!oldData) return oldData;
        
        return {
          ...oldData,
          checklist: oldData.checklist.map((item) =>
