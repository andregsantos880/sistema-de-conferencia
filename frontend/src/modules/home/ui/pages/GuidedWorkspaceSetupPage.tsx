...
import HomeBlurredBackground from '../components/HomeBlurredBackground';
...
const GuidedWorkspaceSetupPage: React.FC = () => {
  ...
  const handleSelectStep = useCallback((stepId: SetupStepId) => {
    setSelectedStepId(stepId);
    setCurrentStep.mutate(stepId);
  }, [setCurrentStep]);

  const handleCompleteStep = useCallback((stepId: SetupStepId) => {
    updateStepStatus.mutate({ stepId, status: 'completed' });
  }, [updateStepStatus]);

  const handleSkipStep = useCallback((stepId: SetupStepId) => {
    updateStepStatus.mutate({ stepId, status: 'skipped' });
  }, [updateStepStatus]);

  const handleStartTour = useCallback((tourId: TourSetId) => {
    openTour(tourId);
  }, [openTour]);
