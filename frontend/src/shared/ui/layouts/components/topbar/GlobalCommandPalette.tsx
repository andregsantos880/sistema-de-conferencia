...
type DialogAction = 'invite-user' | 'create-team' | null;
...
export function GlobalCommandPalette() {
  ...
  const handleSearchSubmit = useCallback(() => {
    if (searchQuery.trim()) {
      navigate(`${SYSTEM_PATHS.SEARCH}?q=${encodeURIComponent(searchQuery.trim())}`);
      handleClose();
    }
  ...
  const commands: PaletteCommand[] = useMemo(() => [
    // Quick Actions
    {
      ...
      action: () => setActiveDialog('invite-user'),
    },
    {
      id: 'create-team',
      ...
      action: () => setActiveDialog('create-team'),
    },
    {
      id: 'send-email',
    ...
    },
    {
      ...
      action: () => navigate('/management/teams'),
    },
    {
      id: 'go-calendar',
  ...
  return (
    <>
      ...
      <button
        ...
        className="flex h-10 w-10 sm:w-auto sm:flex-1 items-center justify-center sm:justify-start gap-2 rounded-xl bg-gray-50 dark:bg-neutral-800 hover:bg-gray-100 dark:hover:bg-neutral-700 border border-gray-200/70 dark:border-neutral-700 sm:px-3 text-sm text-muted-foreground transition-all cursor-pointer group"
        aria-label={t('topbar.search', 'Search')}
      >
        <Search className="h-4 w-4 text-gray-400 group-hover:text-primary transition-colors" />
        <span className="hidden md:inline-block truncate opacity-70">
          {t('topbar.searchPlaceholder', 'Search or type a command...')}
        </span>
        <kbd className="ml-auto hidden lg:inline-flex h-5 items-center gap-1 rounded border bg-white dark:bg-neutral-900 px-1.5 font-mono text-[10px] font-medium text-muted-foreground">
          ⌘K
        </kbd>
      </button>
